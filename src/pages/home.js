...
const Home = (props) => {
  ...
  useEffect(() => {
    ...
    if (props.match.path === '/success' && props.user.authenticated) {
      ...
      showAlert(
        'success',
        `Success!, check "My Gear" section in your account`,
        5
      );
      window.setTimeout(() => {
        props.history.push('/');
  ...
  let productsMarkup = !props.data.loading ? (
    ...
    <div className="spinner--container">
      <Spinner className="spinner-home" size="lg" type="grow" color="secondary" />
    </div>
  );

  return (
    <main className="main">
      <Container>{productsMarkup}</Container>
    </main>
  );
};
